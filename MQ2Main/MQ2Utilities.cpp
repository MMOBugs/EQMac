...
VOID SafeSpewFile(PCHAR szFormat, ...)
{
#ifdef _DEBUG
    va_list vaList;
    va_start( vaList, szFormat );
    int len = _vscprintf( szFormat, vaList )+1;// _vscprintf doesn't count // terminating '\0'
	char *szOutput = (char *)GlobalAlloc(LPTR, len +1);
	if(szOutput) {
		vsprintf_s(szOutput,len,szFormat, vaList);
		FILE *fOut = NULL;
		CHAR Filename[MAX_STRING] = {0};
		sprintf_s(Filename,"%s\\SafeLog.log",gszLogPath);
		errno_t err = 0;
		if((err = fopen_s(&fOut,Filename,"at"))==0) {
			fprintf_s(fOut,"%s\r\n", szOutput);
			fclose(fOut);
		} else {
			//ENOMEM
...
PCHAR GetFullZone(DWORD ZoneID)
{
    ...
    PZONELIST pZone = ((PWORLDDATA)pWorldData)->ZoneArray[ZoneID-1];
...
PITEMINFO GetEnviroContainer()
{
	if (!ppContainerMgr || !pContainerMgr)
		return NULL;
	PEQ_CONTAINERWND_MANAGER ContainerMgr=(PEQ_CONTAINERWND_MANAGER)pContainerMgr;
   if(!ContainerMgr->pWorldItems)
	   return NULL;
   return ContainerMgr->pWorldItems;
}

PEQCONTAINERWINDOW FindContainerForContents(PITEMINFO pContents)
{
   if (!ppContainerMgr || !pContainerMgr)
	   return NULL;
   ...
   {
	   if (pMgr->pPCContainers[j] && (pMgr->pPCContainers[j]->pIteminfo == pContents))
...
VOID GetItemLinkHash(PITEMINFO Item, PCHAR Buffer)
{
	//((EQ_Item*)Item)->GetItemLinkHash(Buffer);
	//0005753Girdle of Flayed Iksar
	sprintf(Buffer,"%06d",Item->ID);
}

VOID GetItemLink(PITEMINFO Item, PCHAR Buffer)
{
	char hash[256];
	//((EQ_Item*)Item)->GetItemLinkHash(hash);
	GetItemLinkHash(Item,hash);

	sprintf(Buffer,"%c0%s%s%c",0x12,hash,Item->Name,0x12);
...
BOOL ItemMatchesSearch(SEARCHITEM &SearchItem, PITEMINFO pContents)
{
	if (SearchItem.ID && pContents->ID!=SearchItem.ID)
		return false;
	RequireFlag(Lore,pContents->Lore);
	RequireFlag(NoRent,pContents->NoRent);
	RequireFlag(NoDrop,pContents->NoDrop);
	RequireFlag(Magic,pContents->Magic);
	RequireFlag(Pack,pContents->Type==ITEMTYPE_PACK);
	RequireFlag(Book,pContents->Type==ITEMTYPE_BOOK);
	RequireFlag(Combinable,pContents->Common.ItemType==17);
	RequireFlag(Summoned,pContents->Summoned);
	RequireFlag(Instrument,pContents->InstrumentType);
	RequireFlag(Weapon,pContents->Common.Damage && pContents->Common.Delay);
	RequireFlag(Normal,pContents->Type==ITEMTYPE_NORMAL);

	CHAR szName[MAX_STRING] = {0};
	if (SearchItem.szName[0] && !strstr(_strlwr(strcpy(szName,pContents->Name)),SearchItem.szName))
		return FALSE;
...
BOOL SearchThroughItems(SEARCHITEM &SearchItem, PITEMINFO* pResult, DWORD *nResult)
{
	// TODO
...
#define Result(pcontents,nresult) 	{\
	...
	if (MaskSet(Worn) && Flag(Worn))
	{
		...
		{
			if (PITEMINFO pContents=pChar->InventoryArray[N])
	...
	if (MaskSet(Inventory) && Flag(Inventory))
	{
		...
		{
			if (PITEMINFO pContents=pChar->Inventory.Pack[nPack])
			{
				if (ItemMatchesSearch(SearchItem,pContents))
		...
		{
			if (PITEMINFO pContents=pChar->Inventory.Pack[nPack])
			if (pContents->Type==ITEMTYPE_PACK)
			{
				for (unsigned long nItem = 0 ; nItem<pContents->Container.Slots ; nItem++)
				{
					if (PITEMINFO pItem=pContents->Container.Contents[nItem])
...
// *************************************************************************** 
PCHAR GetSpellEffectName(WORD EffectID, PCHAR szBuffer) 
{ 
   ...
   case 5: 
      ...
      break; 
   case 32: 
      strcat_s(szBuff,"Summon Item"); 
      break; 
   case 40: 
      strcat_s(szBuff,"Invunerability"); 
      break; 
   case 47: 
      strcat_s(szBuff,"Cold Resist"); 
      break; 
   case 59: 
      strcat_s(szBuff,"Damage Shield"); 
      break; 
   case 69: 
      strcat_s(szBuff,"Max Hitpoints"); 
      break; 
   case 86: 
      strcat_s(szBuff,"Reaction Radius"); 
      break; 
   case 101: 
      strcat_s(szBuff,"Complete Heal (with duration)"); 
      break; 
   case 147: 
      strcat_s(szBuff,"Percentage Heal"); 
      break; 
   default: 
...
{ 
   ...
   { 
      ...
      case 0: //hp +/-: heals/regen/dd 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Hitpoints by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         if ( pSpell->DurationValue1 > 0 ) strcat_s(szBuff, " per tick"); 
         break; 
      case 1: //ac mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "AC by "); 
         SlotValueCalculate(szBuff, pSpell, i, 0.3); 
         break; 
      case 2: //attack mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "ATK by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 3: //movement speed mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Movement by "); 
         if ( pSpell->Max[i] == 0 ) { 
            _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         } else { 
            SlotValueCalculate(szBuff, pSpell, i, 1); 
         } 
         strcat_s(szBuff, "%"); 
      ...
      case 4: //str mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "STR by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 5: //dex mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "DEX by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 6: //agi mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "AGI by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 7: //sta mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "STA by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 8: //int mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "INT by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 9: //wis mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "WIS by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 10: //old lure slot: leaving this since even though patched, som 
         ...
         if ( (abs(pSpell->Base[i]) > 0) && (pSpell->Base[i] != -6) ) { 
            if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
            if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
            strcat_s(szBuff, "CHA by "); 
            SlotValueCalculate(szBuff, pSpell, i, 1); 
         } else { 
            if (pSpell->Base[i] == -6 ){ 
               strcat_s(szBuff, "Lure(6)"); 
            } else { 
               bSlotIsPH=true; 
            } 
         } 
      ...
      case 11: //haste mod 
         ...
         if ( szBase < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( szBase > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Attack Speed by "); 
         _itoa(abs(szBase), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      ...
      case 14: //Water Breathing 
         strcat_s(szBuff, "Water Breathing"); 
         break; 
      case 15: //mana +/- 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Mana by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         if ( pSpell->DurationValue1 > 0 ) strcat_s(szBuff, " per tick"); 
         break; 
      case 18: //Pacify 
         strcat_s(szBuff, "Pacify"); 
         break; 
      case 19: //faction 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Faction by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 20: //Blindness 
         strcat_s(szBuff, "Blindness"); 
         break; 
      ...
      case 24: //Stamina Loss 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Stamina Loss by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 25: //Bind Affinity 
         strcat_s(szBuff, "Bind Affinity"); 
         break; 
      case 26: //Gate 
         strcat_s(szBuff, "Gate"); 
         break; 
      ...
      case 32: //Summon Item 
         ...
         strcat_s(szBuff, "Summon Item: ");   strcat_s(szBuff, szTemp); 
         break; 
      case 33: //Summon Pet 
      ...
      case 35: //disease counters 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Disease Counter by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 36: //poison counters 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Poison Counter by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 40: //Invulnerability 
         strcat_s(szBuff, "Invulnerability"); 
         break; 
      case 41: //Destroy Target 
         strcat_s(szBuff, "Destroy Target"); 
         break; 
      case 42: //Shadowstep(c) 
         ...
         break; 
      case 44: //Lycanthropy 
         strcat_s(szBuff, "Lycanthropy"); 
         break; 
      case 46: //fire resist 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Fire Resist by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      case 47: //cold resist 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Cold Resist by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      case 48: //poison resist 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Poison Resist by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      case 49: //disease resist 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Disease Resist by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      case 50: //magic resist 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Magic Resist by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      case 52: //Sense Undead 
         strcat_s(szBuff, "Sense Undead"); 
         break; 
      case 53: //Sense Summoned 
         strcat_s(szBuff, "Sense Summoned"); 
         break; 
      case 54: //Sense Animals 
         strcat_s(szBuff, "Sense Animals"); 
         break; 
      case 55: //most runes 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Absorb Damage by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      case 56: //True North 
         strcat_s(szBuff, "True North"); 
         break; 
      case 57: //Levitate(c) 
      ...
      case 58: //Illusion: Base=Race 
         strcat_s(szBuff, "Illusion: "); strcat_s(szBuff, pEverQuest->GetRaceDesc(pSpell->Base[i])); 
         break; 
      case 59: //damage shield 
         //base value sign has opposite effect.. 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Damage Shield by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 61: //Identify 
         strcat_s(szBuff, "Identify"); 
         break; 
      case 63: //Memblur e% 
         ...
         break; 
      case 64: //SpinStun 
         strcat_s(szBuff, "SpinStun"); 
         break; 
      case 65: //Infravision 
         strcat_s(szBuff, "Infravision"); 
         break; 
      case 66: //ultravision 
         strcat_s(szBuff, "Ultravision"); 
         break; 
      case 67: //Eye of Zomm 
         strcat_s(szBuff, "Eye Of Zomm"); 
         break; 
      case 68: //reclaim 
         strcat_s(szBuff, "Reclaim Mana"); 
         break; 
      case 69: //max hp mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Max Hitpoints by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      case 71: //Summon Skeleton Pet 
         ...
         break; 
      case 73: //Bind Sight 
         strcat_s(szBuff, "Bind Sight"); 
         break; 
      case 74: //Feign Death 
         strcat_s(szBuff, "Feign Death"); 
         break; 
      case 75: //Voice Graft 
         strcat_s(szBuff, "Voice Graft"); 
         break; 
      case 76: //Sentinel 
         strcat_s(szBuff, "Sentinel"); 
         break; 
      case 77: //Locate Corpse 
         strcat_s(szBuff, "Locate Corpse"); 
         break; 
      case 78: //Absorb Magic Damage 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Absorb Magic Damage by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      case 79: //+hp when cast (priest buffs that have heal component, DoTs with DDs) 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "HP when cast by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      ...
      case 87: //perspective magnification 
         ...
         if ( szBase < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( szBase > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Magnification by "); 
         _itoa(abs(szBase), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      ...
      case 89: //player size 
         ...
         if ( szBase < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( szBase > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Player Size by "); 
         _itoa(abs(szBase), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 90: //Cloak 
         strcat_s(szBuff, "Cloak"); 
         break; 
      case 91: //Summon Corpse 
         strcat_s(szBuff, "Summon Corpse"); 
         break; 
      case 92: //hate mod 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Hate by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 93: //Stop Rain 
         strcat_s(szBuff, "Stop Rain"); 
         break; 
      case 94: //Make Fragile 
         strcat_s(szBuff, "Make Fragile"); 
         break; 
      case 95: //Sacrifice 
         strcat_s(szBuff, "Sacrifice"); 
         break; 
      case 96: //Silence 
         strcat_s(szBuff, "Silence"); 
         break; 
      case 97: //mana pool 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Mana Pool by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 98:  //Haste v2 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Haste v2 by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 99: //Root 
         strcat_s(szBuff, "Root"); 
         break; 
      case 100: //hp mod: pet heals/regen 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Hitpoints v2 by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         if ( pSpell->DurationValue1 > 0 ) strcat_s(szBuff, " per tick"); 
         break; 
      case 101: //Complete Heal (with duration) 
         ...
         break; 
      case 102: //Fearless 
         strcat_s(szBuff, "Fearless"); 
         break; 
      case 103: //Call Pet 
         strcat_s(szBuff, "Call Pet"); 
         break; 
      ...
      case 112: //Effective Casting Level 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Effective Casting Level by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      ...
      case 111: //resist all 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "All Resists by "); 
         SlotValueCalculate(szBuff, pSpell, i , 1); 
         break; 
      case 114: //aggro multiplier 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Aggro Multiplier by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 115: //Food/Water 
         strcat_s(szBuff, "Food/Water"); 
         break; 
      case 116: //curse counters 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Curse Counter by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 117: //Make Weapons Magical 
         strcat_s(szBuff, "Make Weapons Magical"); 
         break; 
      case 118: //Singing Skill 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Singing Skill by "); 
         SlotValueCalculate(szBuff, pSpell, i, 1); 
         break; 
      case 119: //bard overhaste 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Haste v3 by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 120: //Reduce Healing Effectiveness (%) 
         strcat_s(szBuff, "Reduce Healing Effectiveness "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      ...
      case 124: //spell damage 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Spell Damage by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 125: //healing 
      ...
      case 127: //spell haste 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Spell Haste by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 128: //spell duration 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Spell Duration by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 129: //spell range 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Spell Range by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 130: //spell/bash hate 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Spell/Bash Hate by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 131: //Decrease Chance of Using Reagent 
         strcat_s(szBuff, "Decrease Chance of Using Reagent by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 132: //Spell Mana Cost - signs reversed 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Spell Mana Cost by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      ...
      case 135: //Limit: Resist Fire Allowed 
         strcat_s(szBuff, "Limit: Resist Fire allowed"); 
         break; 
      case 136: //limit target types this affects 
         strcat_s(szBuff, "Limit: Target("); 
//		 sprintf(buf, "$spell(%d,targettype)", pSpell->TargetType);
//		 parmSpell(buf, szTemp, NULL );		 // <---- wtf.
//		 strcat_s(szBuff,szTemp);

		 strcat_s(szBuff,"${Spell[%d].TargetType}"); // weeee!

         //GetTargetType(pSpell->Base[i],szTemp);       strcat_s(szBuff,szTemp); 
         if ( pSpell->Base[i] > 0 )  strcat_s(szBuff, " allowed"); 
         if ( pSpell->Base[i] < 0 )  strcat_s(szBuff, " excluded"); 
         strcat_s(szBuff, ")"); 
         break; 
      case 137: //limit effect types this affects 
         strcat_s(szBuff, "Limit: Effect("); 
         GetSpellEffectName(pSpell->Base[i],szTemp);       strcat_s(szBuff,szTemp); 
         if ( pSpell->Base[i] > 0 )  strcat_s(szBuff, " allowed"); 
         if ( pSpell->Base[i] < 0 )  strcat_s(szBuff, " excluded"); 
         strcat_s(szBuff, ")"); 
         break; 
      case 138: //limit spelltype this affects 
         strcat_s(szBuff, "Limit: SpellType("); 
         switch(pSpell->Base[i]) 
         { 
         case 0: 
            strcat_s(szBuff,"Detrimental only"); 
            break; 
         case 1: 
            strcat_s(szBuff,"Beneficial only"); 
            break; 
         case 2: 
      ...
      case 141: //limit to instant spells only 
         strcat_s(szBuff, "Limit: Instant spells only"); 
         break; 
      ...
      case 151: //Suspended Minion 
         strcat_s(szBuff, "Suspended Minion ("); 
         switch(pSpell->Base[i]) 
         { 
         case 0: 
            strcat_s(szBuff,"Current HP Only"); 
            break; 
         case 1: 
            strcat_s(szBuff,"Current HP, Buffs, Weapons"); 
            break; 
         default: 
            sprintf(szTemp, "Unknown%03d", pSpell->Base[i]); 
            strcat_s(szBuff,szTemp); 
      ...
      case 158: //Chance to Reflect Spell 
         if ( pSpell->Base[i] < 0 ) strcat_s(szBuff, "Decrease "); 
         if ( pSpell->Base[i] > 0 ) strcat_s(szBuff, "Increase "); 
         strcat_s(szBuff, "Chance to Reflect Spell by "); 
         _itoa(abs(pSpell->Base[i]), szTemp, 10); strcat_s(szBuff, szTemp); 
         strcat_s(szBuff, "%"); 
         break; 
      case 159: //Stats 
	  ...
	  if ( !bSlotIsPH ) {
		  strcat_s(szBuffer,MAX_STRING, szBuff);
		  strcat_s(szBuffer,MAX_STRING, "<br>" );
	  }
   } 
   return szBuffer;
} 
...
int FindMappableCommand(const char *name)
{
	...
	{
		if(szEQMappableCommands[i]==NULL)
			continue;
...
{
	KeyCombo Ret;
	if (!stricmp(text,"-------") || !stricmp(text,"clear"))//dont want the "clear" here but to be backwardscompatible its a needed evil...
	{
		Dest=Ret;
		return true;
	}
...
PCHAR DescribeKeyCombo(KeyCombo &Combo, PCHAR szDest)
{
	...
	case 0x20:
		...
		break;
	case 0x30:
		strcat(szDest,"shift ctrl+ ");
		break;
	case 0x40:
		strcat(szDest,"alt+ ");
		break;
	case 0x50:
		strcat(szDest,"shift alt+ ");
		break;
	case 0x60:
		strcat(szDest,"ctrl alt+ ");
		break;
	case 0x70:
		strcat(szDest,"shift ctrl alt+ ");
		break;
	}
	if (Combo.Data[0])
	{
		strcat(szDest,gDiKeyName[Combo.Data[0]]);
	}
	else
	{
		strcat(szDest,"-------");
...
int FindInvSlotForContents(PITEMINFO pContents)
{
	DebugSpew("FindInvSlotForContents(0x%08X) (0x%08X)",pContents,pContents->Container);
	PEQINVSLOTMGR pInvMgr=(PEQINVSLOTMGR)*ppInvSlotMgr;
	for (unsigned long N = 0 ; N < 0x1C2 ; N++)
	{
		if (pInvMgr->SlotArray[N] && pInvMgr->SlotArray[N]->pItem)
		{
//			DebugSpew("pInvSlotMgr->SlotArray[%d]->pContents==0x%08X",N,*pInvMgr->SlotArray[N]->ppItems);
			if (pInvMgr->SlotArray[N]->pItem==pContents)
			{
	...
	{
		PCHARINFO pCharInfo=GetCharInfo();
		if (PITEMINFO pPack=pCharInfo->Inventory.Pack[nPack])
		{
			if (pPack->Type==ITEMTYPE_PACK)
			{
				for (unsigned long nItem=0 ; nItem < pPack->Container.Slots ; nItem++)
				{
//					DebugSpew("Pack[%d]->Container.Contents[%d]==0x%08X",nPack,nItem,pPack->Container.Contents[nItem]);
					if (pPack->Container.Contents[nItem]==pContents)
					{
						return 250+(nPack*10)+nItem;
					}
				}
			}
		}
	...
	{
		PCHARINFO pCharInfo=GetCharInfo();
		if (PITEMINFO pPack=pCharInfo->Bank[nPack])
		{
			if (pPack==pContents)
			{
				if (nPack<0x10)
...
int FindInvSlot(PCHAR pName, BOOL Exact)
{
	...
	for (unsigned long N = 0 ; N < 0x1C2 ; N++)
	{
		...
		{
			if (pInvMgr->SlotArray[N]->pItem)
			{
				if (!Exact)
				{
					_strlwr(strcpy(szTemp,pInvMgr->SlotArray[N]->pItem->Name));
				...
				else if (!stricmp(Name,pInvMgr->SlotArray[N]->pItem->Name))
				{
					if (pInvMgr->SlotArray[N]->pInvSlotWnd)
					{
						LastFoundInvSlot=N;
...
int FindNextInvSlot(PCHAR pName, BOOL Exact)
{
	...
	PEQINVSLOTMGR pInvMgr=(PEQINVSLOTMGR)pInvSlotMgr;
	for (unsigned long N = LastFoundInvSlot+1 ; N < 0x1C2 ; N++)
	{
		if (pInvMgr->SlotArray[N])
		{
			if (pInvMgr->SlotArray[N]->pItem)
