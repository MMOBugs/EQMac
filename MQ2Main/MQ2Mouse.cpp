        ...
        return; 
         ...
         return; 
			...
			if (!strnicmp(szArg1, "left", 4) && EnviroTarget.Name[0]!=0 && DistanceToSpawn(pChar,&EnviroTarget)<20.0f) {
				...
				if (pTarget==(EQPlayer*)&EnviroTarget)
					pTarget=NULL;
				return;
			}
      } else if (!strnicmp(szMouseLoc, "door", 4)) {
			// a right clicked door spawn does nothing
			if (!strnicmp(szArg1, "left", 4) && DoorEnviroTarget.Name[0]!=0 && DistanceToSpawn(pChar,&DoorEnviroTarget)<20.0f) {
				EQSwitch *pSwitch = (EQSwitch *)DoorEnviroTarget.Zone;
				pSwitch->UseSwitch(GetCharInfo()->pSpawn->SpawnID,0xFFFFFFFF,0);
				DoorEnviroTarget.Name[0]=0;
				if (pTarget==(EQPlayer*)&DoorEnviroTarget)
					pTarget=NULL;
				return;
			} else {
				DebugSpew("Invalid click args, use \"/click left door\", aborting: %s",szMouseLoc);
			}
