...
{
	//CSidlScreenWnd__GetChildItem
	//CMerchantWnd *merc = ppMerchantWnd;
	CSIDLWND *mgr =  (CSIDLWND *)pMerchantWnd->GetChildItem("MW_SelectedItem");
	if(mgr) {
		Ret.Ptr=*(PITEMINFO *)mgr->TextureAnim;
		Ret.Type=pItemType;
		return true;
	}
	//CSidlScreenWnd*pwnd = CMerchantWnd;
	//pwnd->GetChildItem("MW_SelectedItem")
...
BOOL dataFindItemBank(PCHAR szIndex, MQ2TYPEVAR &Ret)
{
	...
	for (unsigned long nPack=0 ; nPack < NUM_BANK_SLOTS ; nPack++)
	{
		...
		if (PITEMINFO pPack=pCharInfo->Bank[nPack])
		{
			...
			{
				if(strstr(strlwr(strcpy(Temp,pPack->Name)),Name))
				{
					Ret.Ptr=pPack;
					Ret.Type=pItemType;
					return true;
			...
			if (pPack->Type==ITEMTYPE_PACK)
			{
				...
				{
					if (PITEMINFO pItem=pPack->Container.Contents[nItem])
					{
						if (bExact)
						{
							if (!stricmp(Name,pItem->Name))
							{
								Ret.Ptr=pItem;
								Ret.Type=pItemType;
								return true;
						...
						{
							if(strstr(strlwr(strcpy(Temp,pItem->Name)),Name))
							{
								Ret.Ptr=pItem;
								Ret.Type=pItemType;
								return true;
...
BOOL dataFindItem(PCHAR szIndex, MQ2TYPEVAR &Ret)
{
	...
	{
		if (PITEMINFO pItem=pCharInfo->InventoryArray[nSlot])
		{
			if (bExact)
			{
				if (!stricmp(Name,pItem->Name))
			...
			if (bExact)
			{
				if (!stricmp(Name,pItem->Name))
				{
                    if ((pItem->Type != ITEMTYPE_NORMAL) ||
						(pItem->Common.Stackable != 1))
						Count++;
                    else
                        Count+=pItem->Common.StackCount;
				}
			}
			else 
			{
	...
	for (unsigned long nPack=0 ; nPack < NUM_BANK_SLOTS ; nPack++)
	{
		...
		if (PITEMINFO pPack=pCharInfo->Bank[nPack])
		{
			if (bExact)
			{
				if (!stricmp(Name,pPack->Name))
				{
                    if ((pPack->Type != ITEMTYPE_NORMAL) ||
						(pPack->Common.Stackable != 1))
						Count++;
                    else
                        Count+=pPack->Common.StackCount;
				}
			}
			else 
			{
