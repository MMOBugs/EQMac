...
bool MQ2SpawnType::GetMember(MQ2VARPTR VarPtr, PCHAR Member, PCHAR Index, MQ2TYPEVAR &Dest)
{
	...
	case Light:
		...
		return true;
	case State:
		if (pSpawn->pActorInfo && pSpawn->pActorInfo->Stunned==1)
		{
			Dest.Ptr="STUN";
		}
		else
	...
	case AFK:
		...
		return true;
	case LFG:
		Dest.DWord=pSpawn->pActorInfo->LFG;
		Dest.Type=pBoolType;
		return true;
	case Linkdead:
...
//		Dest.DWord=pChar->LDoNPoints;
	...
	*/
	...
	case AAPoints:
		...
		return true;
	case Bound:
		Dest.Ptr = ((PWORLDDATA)pWorldData)->ZoneArray[pChar->ZoneBoundId-1];
		Dest.Type=pZoneType;
		return true; 
	case Combat:
			...
			if (IsNumber(Index))
			{
				...
				if (unsigned long nSkill=atoi(Index))
				{
					...
					else if (nSkill<11)
					{
						...
						return false;
					/**/
                    if (EQADDR_DOABILITYLIST[nSkill]!=0xFFFF)
					{
...
#undef pPetInfoWindow
	...
	case LargestFreeInventory:
		...
		Dest.DWord=0;
		Dest.Type=pIntType;
		for (DWORD slot=22;slot<29;slot++) 
		{
			if (PITEMINFO pItem = pChar->InventoryArray[slot]) 
			{
				if (pItem->Type==ITEMTYPE_PACK && pItem->Container.SizeCapacity>Dest.DWord) 
				{
					for (DWORD pslot=0;pslot<(pItem->Container.Slots);pslot++) 
					{
						if (!pItem->Container.Contents[pslot])
						{
							Dest.DWord=pItem->Container.SizeCapacity;
	...
	case FreeInventory:
		if (Index[0])
		{
			...
			for (DWORD slot=22;slot<29;slot++) 
			{
				if (PITEMINFO pItem = pChar->InventoryArray[slot]) 
				{
					if (pItem->Type==ITEMTYPE_PACK && pItem->Container.SizeCapacity>=nSize) 
					{
						for (DWORD pslot=0;pslot<(pItem->Container.Slots);pslot++) 
						{
							if (!pItem->Container.Contents[pslot]) 
...
{
#define pItem ((PITEMINFO)VarPtr.Ptr)
	if (!VarPtr.Ptr)
		return false;
	...
	case ID:
		...
		return true;
	case Name:
		Dest.Ptr=&pItem->Name[0];
		Dest.Type=pStringType;
		return true;
	case Lore:
		Dest.DWord=(pItem->LoreName[0]=='*');
		Dest.Type=pBoolType;
		return true;
	case NoDrop:
		...
		return true;
	case NoRent:
		Dest.DWord=pItem->NoRent;
		Dest.Type=pBoolType;
		return true;
	case Magic:
		Dest.DWord=((pItem->Type == ITEMTYPE_NORMAL) && (pItem->Common.Magic));
		Dest.Type=pBoolType;
		return true;
	case Value:
		Dest.DWord=pItem->Cost;
		Dest.Type=pIntType;
		return true;
	case Size:
		Dest.DWord=pItem->Size;
		Dest.Type=pIntType;
		return true;
	case Weight:
		Dest.DWord=pItem->Weight;
		Dest.Type=pIntType;
		return true;
	case Stack:
		if ((pItem->Type != ITEMTYPE_NORMAL) || (pItem->Common.Stackable!=1))
			...
			Dest.DWord=pItem->Common.StackCount;
		Dest.Type=pIntType;
		return true;
	case Type:
		if (pItem->Type == ITEMTYPE_NORMAL) 
		{
			...
			{
				Dest.Ptr=szItemTypes[pItem->Common.ItemType];
            } 
			else 
			{
				Dest.Ptr=&DataTypeTemp[0];
                sprintf(DataTypeTemp,"*UnknownType%d",pItem->Common.ItemType);
            }
        }
        else if (pItem->Type == ITEMTYPE_PACK) 
		{
			...
			{
                Dest.Ptr=szCombineTypes[pItem->Container.Combine];
            } 
			else 
			{
				Dest.Ptr=&DataTypeTemp[0];
                sprintf(DataTypeTemp,"*UnknownCombine%d",pItem->Container.Combine);
            }
        }
	...
	case Charges:
		if (pItem->Type != ITEMTYPE_NORMAL)
			...
			Dest.DWord=pItem->Common.Charges;
		Dest.Type=pIntType;
		return true;
	/***case DMGBonusType: well this probably DO exist in eqmac but i havent confirmed needs done.
		Dest.Ptr=szDmgBonusType[pItem->Common.DmgBonusType];
		Dest.Type=pStringType;
		return true;***/
	case Container:
		if (pItem->Type == ITEMTYPE_PACK)
		{
			Dest.DWord=pItem->Container.Slots;
		}
		else
			Dest.DWord=0;
		Dest.Type=pIntType;
	...
	case Items:
		...
		{
			Dest.DWord=0;
			for (unsigned long N=0 ; N < pItem->Container.Slots ; N++)
			{
				if (pItem->Container.Contents[N])
					Dest.DWord++;
			}
			Dest.Type=pIntType;
			return true;
		...
		return false;
	case Item:
		if (pItem->Type == ITEMTYPE_PACK && IsNumber(Index))
		{
			unsigned long N=atoi(Index);
			N--;
			if (N<pItem->Container.Slots)
			{
				if (Dest.Ptr=pItem->Container.Contents[N])
				{
					Dest.Type=pItemType;
					return true;
				}
		...
		return false;
	case Stackable:
		Dest.DWord=pItem->Common.Stackable;
		Dest.Type=pBoolType;
		return true;
	case InvSlot:
	...
	case SellPrice:
		if (pActiveMerchant)
		{
			Dest.DWord=(DWORD)((FLOAT)pItem->Cost*(1.0f/((PEQMERCHWINDOW)pMerchantWnd)->Markup));
			Dest.Type=pIntType;
			return true;
		}
		return false;
	case WornSlot:
		if (Index[0])
		{
			if (IsNumber(Index))
			{
				...
				if (!Count)
					return false;
				DWORD cmp=pItem->EquipableSlots;
			...
			else
			{
				...
				{
					Dest.DWord=(pItem->EquipableSlots&(1<<nInvSlot));
					Dest.Type=pBoolType;
					return true;
				}
			}
	...
	case CastTime:
		Dest.Float=(FLOAT)pItem->Common.CastTime/1000;
		Dest.Type=pFloatType;
		return true;
	case Spell:
		if (Dest.Ptr=GetSpellByID(pItem->Common.SpellId))
		{
			Dest.Type=pSpellType;
			return true;
		}
	...
	case EffectType:
		...
		// 3 = ?
		// 4 = click worn
		if (GetSpellByID(pItem->Common.SpellId))
		{
			switch(pItem->Common.EffectType)
			{
			case 0:
				Dest.Ptr="Combat";
				break;
		...
		return false;
	case InstrumentMod:
		Dest.Float=((FLOAT)pItem->Common.InstrumentMod)/10.0f;
		Dest.Type=pFloatType;
		return true;
	/*case Tribute:
		Dest.DWord=pItem->Common.Favor;
		Dest.Type=pIntType;
		return true;*/
	/*
		Haste=16,
...
#define pLoot ((PEQLOOTWINDOW)pLootWnd)
	...
	case Item:
		if (Index[0])
		{
			...
			else
			{
				...
				strlwr(pName);
				CHAR Temp[MAX_STRING]={0};
				for (unsigned long nIndex = 0 ; nIndex < MAX_INV_SIZE ; nIndex++)
				{
					if (PITEMINFO pContents=pLoot->ItemDesc[nIndex])
					{
						if (bExact)
						{
							if (!stricmp(pName,pContents->Name))
							{
								Dest.Ptr=pContents;
								Dest.Type=pItemType;
								return true;
						...
						{
							if(strstr(strlwr(strcpy(Temp,pContents->Name)),pName))
							{
								Dest.Ptr=pContents;
								Dest.Type=pItemType;
								return true;
	...
	case Items:
		{
			Dest.DWord=0;
			for (unsigned long N = 0 ; N < MAX_INV_SIZE ; N++)
			{
				if (pLoot->ItemDesc[N])
					Dest.DWord++;
			}
	...
	case Item:
		if (Index[0])
		{
			...
			else
			{
				...
				{
					if (PITEMINFO pContents=pMerch->ItemDesc[nIndex])
					{
						if (bExact)
						{
							if (!stricmp(pName,pContents->Name))
...
#define nInvSlot (VarPtr.Int)
	...
	case Item:
		{
			...
			{
				if (((PEQINVSLOT)pSlot)->pItem)
				{
					//if (Dest.Ptr=*((PEQINVSLOT)pSlot)->pItem)
					if (Dest.Ptr=((PEQINVSLOT)pSlot)->pItem)
					{
						Dest.Type=pItemType;
						return true;
			...
			else
			{
				...
				if (nInvSlot>=251 && nInvSlot<361)
				{
					...
					if (pPack->Type==ITEMTYPE_PACK && nSlot<pPack->Container.Slots)
					{
						if (Dest.Ptr=pPack->Container.Contents[nSlot])
						{
							Dest.Type=pItemType;
							return true;
						}
	...
	case Slot:
		...
		return false;
	case Name:
		if (nInvSlot>=0 && nInvSlot<MAX_INV_SIZE) 
        {
            Dest.Ptr=szItemSlot[nInvSlot];
            Dest.Type=pStringType;
            return true;
        }
		if (nInvSlot>=22 && nInvSlot<MAX_INV_SIZE)
		{
		...
		else if (nInvSlot>=2000 && nInvSlot<2008)
		{
