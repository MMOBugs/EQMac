...
VOID Doors(PSPAWNINFO pChar, PCHAR szLine)
{
   ...
   if (!pSwitchMgr) return;
       PDOOR pDoorTable = (PDOOR)pSwitchMgr;
   DWORD ActualCount=0;
   CHAR szBuffer[MAX_STRING] = {0};
   ...
   while (pDoorTable && pDoorTable->pNext) {
      if ((szLine[0]==0) || (!strnicmp(pDoorTable->Name,szLine,slen))) {
         SPAWNINFO TempSpawn;
         FLOAT Distance;
         ZeroMemory(&TempSpawn,sizeof(TempSpawn));
         strcpy(TempSpawn.Name,pDoorTable->Name);
         TempSpawn.Y=pDoorTable->Y;
         TempSpawn.X=pDoorTable->X;
         TempSpawn.Z=pDoorTable->Z;
         TempSpawn.Heading=pDoorTable->Heading;
         Distance = DistanceToSpawn(pChar,&TempSpawn);
         INT Angle = (INT)((atan2f(pChar->X - pDoorTable->X, pChar->Y - pDoorTable->Y) * 180.0f / PI + 360.0f) / 22.5f + 0.5f) % 16;
         sprintf(szBuffer,"%d: %s: %1.2f away to the %s",pDoorTable->ID, pDoorTable->Name, Distance, szHeading[Angle]);
         WriteChatColor(szBuffer,USERCOLOR_DEFAULT);
         ActualCount++;
		 pDoorTable=pDoorTable->pNext;
      }
   }
...
VOID DoorTarget(PSPAWNINFO pChar, PCHAR szLine)
{
   ...
   if (!stricmp(Arg1, "id")) {
	  ...
	  while (pDoorTable && pDoorTable->pNext) {
         if (pDoorTable->ID == ID) {
            strcpy(DoorEnviroTarget.Name, pDoorTable->Name);
            DoorEnviroTarget.Y = pDoorTable->Y;
            DoorEnviroTarget.X = pDoorTable->X;
            DoorEnviroTarget.Z = pDoorTable->Z;
            DoorEnviroTarget.Heading = pDoorTable->Heading;
			...
			DoorEnviroTarget.Zone = (DWORD)pDoorTable;
            pDoorTarget = pDoorTable;
            break;
         }
		 pDoorTable=pDoorTable->pNext;
      }
      ...
      while (pDoorTable && pDoorTable->pNext) {
         if (((szSearch[0]==0) ||
             (!strnicmp(pDoorTable->Name,szSearch,strlen(szSearch)))) &&
            ((gZFilter >=10000.0f) ||
              ((pDoorTable->Z <= pChar->Z + gZFilter) &&
               (pDoorTable->Z >= pChar->Z - gZFilter)))) {
            SPAWNINFO tSpawn;
            ZeroMemory(&tSpawn,sizeof(tSpawn));
            strcpy(tSpawn.Name,pDoorTable->Name);
            tSpawn.Y=pDoorTable->Y;
            tSpawn.X=pDoorTable->X;
            tSpawn.Z=pDoorTable->Z;
			tSpawn.Type = SPAWN_NPC;
			tSpawn.HPCurrent = 1;
			tSpawn.HPMax = 1;
			tSpawn.pActorInfo = &EnviroActor;
            tSpawn.Heading=pDoorTable->Heading;
			tSpawn.Zone = (DWORD)pDoorTable;
            FLOAT Distance = DistanceToSpawn(pChar,&tSpawn);
            if (Distance<cDistance) {
...
VOID UpdateItemInfo(PSPAWNINFO pChar, PCHAR szLine) {
   CHAR szBuffer[MAX_STRING] = {0};
   PITEMINFO pContainer = NULL;
   int nInvIdx;
   PCHARINFO pCharInfo = NULL;
   if (NULL == (pCharInfo = GetCharInfo())) return;

   for (nInvIdx=0; nInvIdx < MAX_INV_SIZE; nInvIdx++) {
      if (pCharInfo->InventoryArray[nInvIdx] != NULL) {
         BOOL Found = FALSE;
         PITEMDB ItemDB = gItemDB;
         while (ItemDB) {
			 if (ItemDB->ID == pCharInfo->InventoryArray[nInvIdx]->ID) {
               Found = TRUE;
            }
            ItemDB = ItemDB->pNext;
         }
         if (!Found) {
            ...
            Item->pNext = gItemDB;
            Item->ID = pCharInfo->InventoryArray[nInvIdx]->ID;
            strcpy(Item->szName, pCharInfo->InventoryArray[nInvIdx]->Name);
            DebugSpew("   New Item found - %d: %s", Item->ID, Item->szName);
            gItemDB = Item;
         }
         if (pCharInfo->InventoryArray[nInvIdx]->Type == ITEMTYPE_PACK) {
			 pContainer = pCharInfo->InventoryArray[nInvIdx];
            DebugSpew("   Opening Pack");
            for (int nPackIdx = 0; nPackIdx < pCharInfo->InventoryArray[nInvIdx]->Container.Slots; nPackIdx++) {
               if (pContainer->Container.Contents[nPackIdx] != NULL) {
                  Found = FALSE;
                  PITEMDB ItemDB = gItemDB;
                  while (ItemDB) {
                      if (pContainer->Container.Contents[nPackIdx]) {
                        if (ItemDB->ID == pContainer->Container.Contents[nPackIdx]->ID) {
                            Found = TRUE;
                        }
                        ItemDB = ItemDB->pNext;
                      }
                  ...
                  if (!Found) {
                     PITEMDB Item = (PITEMDB)malloc(sizeof(ITEMDB));
                     Item->pNext = gItemDB;
                     Item->ID = pContainer->Container.Contents[nPackIdx]->ID;
                     strcpy(Item->szName, pContainer->Container.Contents[nPackIdx]->Name);
                     DebugSpew("      New Item found - %d: %s", Item->ID, Item->szName);
                     gItemDB = Item;
                  }
               }
   ...
   for (nInvIdx=0; nInvIdx < NUM_BANK_SLOTS; nInvIdx++) {
      if (pCharInfo->Bank[nInvIdx] != NULL) {
         ...
         PITEMDB ItemDB = gItemDB;
         while (ItemDB) {
            if (ItemDB->ID == pCharInfo->Bank[nInvIdx]->ID) {
               Found = TRUE;
            }
            ItemDB = ItemDB->pNext;
         }
         if (!Found) {
            PITEMDB Item = (PITEMDB)malloc(sizeof(ITEMDB));
            Item->pNext = gItemDB;
            Item->ID = pCharInfo->Bank[nInvIdx]->ID;
...
DWORD __stdcall WaitForSpellMemming(PVOID pData)
{
	if(!pSpellBookWnd)
		return 0;
	PSPELLFAVORITE fav=(PSPELLFAVORITE)pData;
	WORD Gem = fav->Byte_39;
	WORD SpellID = (WORD)fav->SpellId[0];
	if(Gem>7 || Gem<0)
		return 0;
	int ticks = 10000;
	for(int i=0;i<1200;i++) {//2 mins? even noobs gets it done in that time I hope...
		ticks = pSpellBookWnd->GetSpellMemTicksLeft();
		if(ticks==0)
			break;
		Sleep(100);
	}
	if(fav->Byte_3B==0) {
		((CXWnd*)pSpellBookWnd)->Deactivate();
		//((CXWnd*)pSpellBookWnd)->Show(0,1);
	}
	if(fav->Byte_3A!=0) {
		if(GetCharInfo()->standstate!=STANDSTATE_STAND)
			DoCommand(GetCharInfo()->pSpawn,"/stand");
	}
	return 1;
}
// ***************************************************************************
...
VOID MemSpell(PSPAWNINFO pChar, PCHAR szLine)
{
    if (!ppSpellBookWnd)
		return;
	while(pSpellBookWnd->GetSpellMemTicksLeft())
		Sleep(100);
   ...
   if (Gem<1 || Gem>8)
	   return;
   Gem--;
	if(pCharInfo->MemorizedSpells[Gem]!=0xFFFF)
		return;

   pCharInfo->SpellBook;
   PSPELL pSpell=0;
   WORD N = 0;
	for ( ; N < NUM_BOOK_SLOTS ; N++) {
    ...
    if (pSpell->Level[pChar->Class-1]>pChar->Level)
		return;
	MemSpellFavorite.Byte_3B=pSpellBookWnd->dShow;
	if(pSpellBookWnd->dShow==0) {
		pSpellBookWnd->Activate();
		//((CXWnd*)pSpellBookWnd)->Show(1,1);
	}
	if(pChar->StandState!=STANDSTATE_SIT && !pChar->pActorInfo->Mount)
	{
		MemSpellFavorite.Byte_3A=pChar->StandState;
		DoCommand(pChar,"/sit");
	} else if(pChar->pActorInfo->Mount) {
		MemSpellFavorite.Byte_3A=0;
	}
	pSpellBookWnd->StartSpellMemorization(N,Gem,0);
	MemSpellFavorite.SpellId[0]=pSpell->ID;
	MemSpellFavorite.Byte_39=(BYTE)Gem;
	DWORD nThreadID = 0;
	CreateThread(NULL,0,WaitForSpellMemming,&MemSpellFavorite,0,&nThreadID);
}
...
VOID Filter(PSPAWNINFO pChar, PCHAR szLine)
{
    ...
    if (!stricmp("skills",szArg)) {
        ...
        if (szRest[0]==0) {
            sprintf(szCmd,"Filtering of MQ2 is set to: %s",szUseChat[gFilterMQ]);
            WriteChatColor(szCmd,USERCOLOR_DEFAULT);
            return;
        }
...
VOID Identify(PSPAWNINFO pChar, PCHAR szLine)
{
	...
	DebugSpew("Identify - %s", pCharInfo->Cursor->LoreName);
    WriteChatColor(" ",USERCOLOR_SPELLS);
	if		( pCharInfo->Cursor->Type == ITEMTYPE_NORMAL && pCharInfo->Cursor-> Common.ItemType < MAX_ITEMTYPES && szItemTypes[pCharInfo->Cursor->Common.ItemType] != NULL  )
	    sprintf(szMsg,"Item: %s (Slot: %s, Weight: %d.%d, Value: %dcp, Type: %s)",pCharInfo->Cursor->Name,szSize[pCharInfo->Cursor->Size], (INT)(pCharInfo->Cursor->Weight/10),(pCharInfo->Cursor->Weight) % 10, pCharInfo->Cursor->Cost, szItemTypes[pCharInfo->Cursor->Common.ItemType] );
	else if ( pCharInfo->Cursor->Type == ITEMTYPE_PACK && pCharInfo->Cursor->Container.Combine < MAX_COMBINES && szCombineTypes[pCharInfo->Cursor->Container.Combine] != NULL )
		sprintf(szMsg,"Item: %s (Slot: %s, Weight: %d.%d, Value: %dcp, Type: %s)",pCharInfo->Cursor->Name,szSize[pCharInfo->Cursor->Container.SizeCapacity], (INT)(pCharInfo->Cursor->Weight/10),(pCharInfo->Cursor->Weight) % 10, pCharInfo->Cursor->Cost, szCombineTypes[pCharInfo->Cursor->Container.Combine] );
    else
		sprintf(szMsg,"Item: %s (Slot: %s, Weight: %d.%d, Value: %dcp)",pCharInfo->Cursor->Name,szSize[pCharInfo->Cursor->Size], (INT)(pCharInfo->Cursor->Weight/10),(pCharInfo->Cursor->Weight) % 10, pCharInfo->Cursor->Cost );


    WriteChatColor(szMsg,USERCOLOR_SPELLS);
	if ((pCharInfo->Cursor->LoreName[0] != '*') && (strcmp(pCharInfo->Cursor->LoreName,pCharInfo->Cursor->Name))) {
		sprintf(szMsg,"Lore Name: %s",pCharInfo->Cursor->LoreName);
        WriteChatColor(szMsg,USERCOLOR_SPELLS);
	} else 	if ((pCharInfo->Cursor->LoreName[0] == '*') && (strcmp(pCharInfo->Cursor->LoreName+1,pCharInfo->Cursor->Name))) {
		sprintf(szMsg,"Lore Name: %s",pCharInfo->Cursor->LoreName+1);
        WriteChatColor(szMsg,USERCOLOR_SPELLS);
    }

    strcpy(szMsg,"Flags: ");
	if (pCharInfo->Cursor->LoreName[0] == '*') strcat_s(szMsg,"LORE ");
	if (pCharInfo->Cursor->NoDrop == 0) strcat_s(szMsg,"NODROP ");
	if (pCharInfo->Cursor->NoRent == 0) strcat_s(szMsg,"NORENT ");
	if (pCharInfo->Cursor->Type == ITEMTYPE_NORMAL) {
		if (pCharInfo->Cursor->Magic == 1)
			strcat_s(szMsg,"MAGIC ");
		BYTE Light = pCharInfo->Cursor->Common.Light;
    ...
    if (strlen(szMsg) > 7)
		WriteChatColor(szMsg,USERCOLOR_SPELLS);
	strcpy(szMsg,"Spell Effect: ");
	if (pCharInfo->Cursor->Common.SpellId != 0) {
		PSPELL pSpell = GetSpellByID(pCharInfo->Cursor->Common.SpellId);
		if(pSpell)
			strcat_s(szMsg,pSpell->Name);
	}
	if (strlen(szMsg) > 14)
		WriteChatColor(szMsg,USERCOLOR_SPELLS);
	if (pCharInfo->Cursor->Type == ITEMTYPE_PACK) {
        CHAR szCombine[MAX_STRING] = {0};
		if ((pCharInfo->Cursor->Container.Combine < MAX_COMBINES) && (szCombineTypes[pCharInfo->Cursor->Container.Combine] != NULL)) {
			strcpy(szCombine,szCombineTypes[pCharInfo->Cursor->Container.Combine]);
        } else {
			sprintf(szCombine,"*Unknown%d",pCharInfo->Cursor->Container.Combine);
        }
		sprintf(szMsg,"Container: %d Slot %s, %d%% Reduction, Combine=%s",pCharInfo->Cursor->Container.Slots,szSize[pCharInfo->Cursor->Container.SizeCapacity],pCharInfo->Cursor->Container.WeightReduction,szCombine);
	} else if (pCharInfo->Cursor->Type == ITEMTYPE_BOOK) {
//		sprintf(szMsg,"Book file: %s", pCharInfo->Cursor->Item->Book.File);
    } else {
            strcpy(szMsg,"Item: ");
			if (pCharInfo->Cursor->Common.AC) {
				sprintf(szTmp,"AC%d ",pCharInfo->Cursor->Common.AC);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.Damage) {
				sprintf(szTmp,"%dDam ",pCharInfo->Cursor->Common.Damage);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.Delay) {
				sprintf(szTmp,"%dDly ",pCharInfo->Cursor->Common.Delay);
                strcat_s(szMsg,szTmp);
            }
			
			if (pCharInfo->Cursor->Common.Range) {
				sprintf(szTmp,"%dRng ",pCharInfo->Cursor->Common.Range);
                strcat_s(szMsg,szTmp);
            }
			if ((pCharInfo->Cursor->Common.Skill)) {
				sprintf(szTmp,"%s ",szSkills[pCharInfo->Cursor->Common.Skill]);
                //strcat_s(szMsg,szTmp);
            }
			
			if (pCharInfo->Cursor->Common.HP) {
				sprintf(szTmp,"%dHP ",pCharInfo->Cursor->Common.HP);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.Mana) {
				sprintf(szTmp,"%dMana ",pCharInfo->Cursor->Common.Mana);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.STR) {
				sprintf(szTmp,"%dSTR ",pCharInfo->Cursor->Common.STR);
                strcat_s(szMsg,szTmp) ;
            }
			if (pCharInfo->Cursor->Common.STA) {
				sprintf(szTmp,"%dSTA ",pCharInfo->Cursor->Common.STA);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.DEX) {
				sprintf(szTmp,"%dDEX ",pCharInfo->Cursor->Common.DEX);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.AGI) {
				sprintf(szTmp,"%dAGI ",pCharInfo->Cursor->Common.AGI);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.WIS) {
				sprintf(szTmp,"%dWIS ",pCharInfo->Cursor->Common.WIS);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.INT) {
				sprintf(szTmp,"%dINT ",pCharInfo->Cursor->Common.INT);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.CHA) {
				sprintf(szTmp,"%dCHA ",pCharInfo->Cursor->Common.CHA);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.SvMagic) {
				sprintf(szTmp,"%dSvM ",pCharInfo->Cursor->Common.SvMagic);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.SvDisease) {
				sprintf(szTmp,"%dSvD ",pCharInfo->Cursor->Common.SvDisease);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.SvPoison) {
				sprintf(szTmp,"%dSvP ",pCharInfo->Cursor->Common.SvPoison);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.SvFire) {
				sprintf(szTmp,"%dSvF ",pCharInfo->Cursor->Common.SvFire);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.SvCold) {
				sprintf(szTmp,"%dSvC ",pCharInfo->Cursor->Common.SvCold);
                strcat_s(szMsg,szTmp);
            }
			if (pCharInfo->Cursor->Common.Stackable==1) {
				sprintf(szTmp,"Stack size = %d ",pCharInfo->Cursor->Common.StackCount);
                strcat_s(szMsg,szTmp);
            }
    }
	...
	CHAR szTemp[MAX_STRING]={0};
        ...
        if (pSpawn->Type == SPAWN_PLAYER) {
            ...
            if (gFilterSWho.Guild && pSpawn->GuildID < MAX_GUILDS && pGuildList) {
                strcat_s(szName," <");
                strcat_s(szName,GetGuildByID(pSpawn->GuildID));
                strcat_s(szName,">");
            }
        } else {
            CleanupName(szName);
		...
		if (gFilterSWho.Level || gFilterSWho.Race || gFilterSWho.Body || gFilterSWho.Class) {
			...
			if (gFilterSWho.Body) {
				strcat_s(szMsg,GetBodyTypeDesc(pSpawn->BodyType));
				strcat_s(szMsg," ");
			}
...
//		strcat_s(szMsg,"\ax");
        ...
        if (pSpawn->Type == SPAWN_PLAYER) {
			...
			if (gFilterSWho.LFG && pSpawn->pActorInfo->LFG) strcat_s(szMsg," \ag<LFG>\ax");
            if (gFilterSWho.Trader && pSpawn->pActorInfo->Trader) strcat_s(szMsg," \ag<Trader>\ax");
...
VOID DoAbility(PSPAWNINFO pChar, PCHAR szLine)
{
    ...
    if (!stricmp(szBuffer,"list")) {
        ...
        for (Index=4;Index<10;Index++) {
            if (EQADDR_DOABILITYLIST[Index]==0xFFFF) {
...
{
	WriteChatf("Not implemented yet, check back later");
	return;
...
if (!stricmp(szArg1,"item"))
   {
      ...
      for (int i=0;i<MAX_INV_SIZE;i++) {
         if (pCharInfo->InventoryArray[i])
            if (!_stricmp(szArg2,pCharInfo->InventoryArray[i]->Name)) { 
            DebugSpew("cast test slot %d = %s address is %x",i,pCharInfo->InventoryArray[i]->Name,&(pCharInfo->InventoryArray[i])); 
...
PCHAR FormatSearchSpawn(PCHAR Buffer, PSEARCHSPAWN pSearchSpawn)
{
    ...
    if (pSearchSpawn->bGM && pSearchSpawn->SpawnType != NPC ) strcat_s(Buffer,128," GM");
    if (pSearchSpawn->bTrader) strcat_s(Buffer,128," Trader");
    if (pSearchSpawn->bLFG) strcat_s(Buffer,128," LFG");
	if (pSearchSpawn->bLight) {
		strcat_s(Buffer,128," Light");
		if (pSearchSpawn->szLight[0]) {
			strcat_s(Buffer,128,":");
			strcat_s(Buffer,128,pSearchSpawn->szLight);
		}
	}
...
BOOL SpawnMatchesSearch(PSEARCHSPAWN pSearchSpawn, PSPAWNINFO pChar, PSPAWNINFO pSpawn)
{
	...
	if (pSearchSpawn->bSlower && pSearchSpawn->SpawnType != NPC)
		if (pSpawn->Class != 10 && pSpawn->Class != 14 && pSpawn->Class != 15)
			return FALSE;
	if (pSearchSpawn->bLFG && !pSpawn->pActorInfo->LFG)
		return FALSE;
...
VOID SetAutoRun(PSPAWNINFO pChar, PCHAR szLine)
{
	...
	sprintf(szServerAndName,"%s.%s",((PEVERQUEST)pinstCEverQuest)->ChatService->ServerName,((PCHARINFO)pCharData)->Name);
...
VOID BankList(PSPAWNINFO pChar, PCHAR szLine)
{
    ...
    for (int a=0;a<8;a++) {
        ...
        if (pContainer) {
			...
			sprintf(szTemp,"Slot %d: %dx %s (%s)",a,pContainer->Common.StackCount ? pContainer->Common.StackCount : 1,Link,pContainer->LoreName);
            WriteChatColor(szTemp,USERCOLOR_DEFAULT);
            for (int b=0;b<pContainer->Container.Slots;b++) {
				if (pContainer->Container.Contents[b]) {
					GetItemLink(pContainer->Container.Contents[b],&Link[0]);
                    sprintf(szTemp,"- Slot %d: %dx %s (%s)",b,pContainer->Container.Contents[b]->Common.StackCount ? pContainer->Container.Contents[b]->Common.StackCount : 1,Link,pContainer->Container.Contents[b]->LoreName);
                    WriteChatColor(szTemp,USERCOLOR_DEFAULT);
                }
            }
        }
...
VOID DoMappable(PSPAWNINFO pChar, PCHAR szLine)
{
	...
	if (!PressMQ2KeyBind(szArg1,Hold))
	{
		...
		if (ParseKeyCombo(szArg1,Temp))
		{
			...
			{
				pWndMgr->HandleKeyboardMsg(Temp.GetKey(),1);
				pWndMgr->HandleKeyboardMsg(Temp.GetKey(),0);
			}
			else
			{
				MQ2HandleKeyDown(Temp,1);
				if (!Hold)
					MQ2HandleKeyUp(Temp,1);
			}
			return;
		}
...
VOID EnviroCloseCmd(PSPAWNINFO pChar, PCHAR szLine)
{
	CXWnd *pWnd=FindMQ2Window("enviro");
	if (!pWnd)
	{
		MacroError("Couln't find an open worldcontainer",szLine);
		return;
	}
	...
	CXWnd *envwnd = (CXWnd *)((PEQCONTAINERWINDOW)pWnd)->pDone;
	DebugTry(CXRect rect= envwnd->GetScreenRect());
	DebugTry(CXPoint pt=rect.CenterPoint());
	DebugTry(envwnd->HandleLButtonDown(pt,0x20000));
	DebugTry(envwnd->HandleLButtonUp(pt,0));
}
VOID CombineCmd(PSPAWNINFO pChar, PCHAR szLine)
{
	CHAR szArg2[MAX_STRING] = {0}; 
	GetArg(szArg2, szLine, 0,0);
	if (!szLine[0])
	{
		SyntaxError("Usage: /combine <pack>");
		return;
	}
	CXWnd *pWnd=FindMQ2Window(szArg2);
	if (!pWnd)
	{
		MacroError("Window for '%s' not found",szArg2);
		return;
	}
	if ((DWORD)pWnd->pvfTable!=CContainerWnd__vftable)
	{
		MacroError("Window '%s' is not a container window",szArg2);
		return;
	}
