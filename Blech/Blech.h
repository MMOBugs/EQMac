...
//		ExactMatch.clear();
	...
	unsigned long AddExactEvent(char *Text,fBlechCallback Callback,void *pData=0)
	{
		...
		pEvent->ID=++LastID;
		pEvent->pBlechNode=0;
		pEvent->OriginalString=_strdup(Text);
...
feedermatchdoevents:
			...
			for (unsigned long N = PLP-1 ; N > 0 ; N--)
			{
				if (MatchStack[N].pNode && MatchStack[N].pNode->StringType==BST_SCANVAR)
				{
					...
					{
//						if (N)
//							pValue->Value=_strdup(MatchStack[N].Pos+MatchStack[N-1].pNode->Length);
//						else
							pValue->Value=_strdup(MatchStack[N].Pos);
					}
